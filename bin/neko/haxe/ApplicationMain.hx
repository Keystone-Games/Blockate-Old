...
class ApplicationMain
{
	...
	{
		lime.system.System.__registerEntryPoint("Blockate", create);

		#if (js && html5)
		#if (munit || utest)
		lime.system.System.embed("Blockate", null, 1280, 720);
		#end
		#else
		create(null);
		#end
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("blockate.Main")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess blockate.Main.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"blockate.Main\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"blockate.Main\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends blockate.Main {}
#else
class DocumentClass
{
